...
public class OAuthService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        Member member = saveOrUpdate(memberProfile);      //DB에 저장
    ...
    private Member saveOrUpdate(MemberProfile memberProfile){
        ...
        Member member = memberRepository.findByOauthId(oAuthId)
                .map(m -> m.update((String) memberProfile.getAttributes().get("email"))) //OAuth 서비스 유저정보 변경이 있으면 업데이트
                .orElse(memberProfile.toUser());          //user가 없으면 새로운 user 생성
